...
class SchemeDisplayAgent:
    def __init__(self, schemes: List[Dict], user_profile: dict = None):
        ...
        user_profile_str = json.dumps(self.user_profile) if self.user_profile else "{}"
        user_profile_str = user_profile_str.replace("{", "{{").replace("}", "}}")

        self.prompt_template = PromptTemplate(
            input_variables=["history", "input", "scheme_details"],
            template=f"""You are a Government Scheme Assistant. The user's profile is: {user_profile_str}. You MUST cross-reference this profile with the scheme requirements. ONLY recommend schemes they are explicitly eligible for.
When suggesting eligible schemes, you MUST format the options as a simple numbered list with just the scheme names (e.g., 1. Scheme Name A \n 2. Scheme Name B). Do not use bolding (**).

You are a helpful assistant guiding the user through government schemes in a natural, conversational way. Use the conversation history, user input, and scheme details to respond appropriately. Format the scheme details in a structured way as requested by the user.
...
"""
    ...
    def display_schemes(self) -> str:
        ...
        for i, scheme in enumerate(self.schemes, 1):
            metadata = scheme.get('metadata', {})
            scheme_list += f"{i}. {metadata.get('scheme_name', 'Unknown')}\n"
            scheme_list += f"   Brief Description: {metadata.get('brief_description', 'N/A')}\n"
            scheme_list += f"   State: {metadata.get('state', 'N/A')}\n\n"
    ...
    def format_scheme_details(self) -> str:
        ...
        details = (
            f"Scheme Name: {self.scheme_details.get('scheme_name', 'N/A')}\n"
            f"Detailed Description: {self.scheme_details.get('detailed_description', 'N/A')}\n"
            f"Eligibility Criteria: {self.scheme_details.get('eligibility_criteria', 'N/A')}\n"
            f"Application Process: {self.scheme_details.get('application_process', 'N/A')}\n"
            f"Documents Required: {self.scheme_details.get('documents_required', 'N/A')}"
        )
        return details
        ...
        if self.state == "list_schemes":
            ...
            try:
                ...
                if 1 <= choice <= len(self.schemes):
                    ...
                    metadata = self.selected_scheme.get('metadata', {})
                    scheme_id = metadata.get('scheme_id')
                    self.scheme_details = fetch_scheme_details(scheme_id) if scheme_id else None
                ...
                scheme_details_str = "None selected yet."
