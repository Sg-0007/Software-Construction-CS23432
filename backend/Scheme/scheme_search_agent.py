...
def load_user_details(json_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Define required fields (using .get() later for safety)
        ...
        if missing_fields:
            ...
            # We'll still try to proceed or raise error as before
            # raise ValueError(f"Missing required fields: {missing_fields}")
            
        sanitized_data = {k: sanitize_input(str(v)) for k, v in data.items() if v is not None}
        
        # Standardize name/full_name
        sanitized_data['full_name'] = sanitized_data.get('full_name', sanitized_data.get('name', 'User'))
...
def rerank_with_llm(
    ...
    # Debug: Print user details used for re-ranking
    print(f"DEBUG: Re-ranking for user: {user_details.get('full_name', 'User')}")
...
def main():
    ...
    try:
        ...
        for i, scheme in enumerate(schemes, 1):
            ...
            print(f"\n{i}. {metadata.get('scheme_name', 'Unknown')}")
            print(f"   Description: {metadata.get('brief_description', 'No description available')}")
            print(f"   LLM Score: {scheme.get('llm_score', 0)}, Pinecone Score: {scheme.get('pinecone_score', 0)}")
    except FileNotFoundError:
